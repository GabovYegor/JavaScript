$(document).ready(()=>{
    $(document).on('click', '#addPictureBtn', function () {
        if($(this).hasClass('addActive')){
            ...
            $(this).after('<div id="addForm">\n' +
                ...
                '  <button id="addPictureFormBtn">Add picture</button>' +
                '  </div> ')
        }
    })
    ...
    $(document).on('click', '#addPictureFormBtn', function () {
        ...
        var response = $.post( "/addPictureAction", {title: $('#addTitle').val(), author: $('#addAuthor').val(), image: $('#addImage').val(),
                                                     description: $('#addDescription').val(), startPrice: $('#addStartPrice').val() }, ()=>{}, "json")
        imagePath = '/public/images/' + $('#addImage').val()
        $('ol').append('<li><img src="' + JSON.parse(response.responseText).imagePath + '"></img>Author: ' + $('#addAuthor').val()
                       + 'Title: ' + $('#addTitle').val() + '<br><a href="/loadPictureCard/'
                       +  JSON.parse(response.responseText).id +'">Go to picture card</a><br><br><br><br><br></li>')
    })
})
