$(document).ready(() => {
  $('li').click(function () {
    if (!$(this).hasClass('selected')) {
      ...
      $(this).append('<div id="changeForm">\n' + '  <input type="text" placeholder="new userName" id="newUserName"><br>\n' + '  <input type="text" placeholder="new amount of money" id="newAmountOfMoney"><br>\n' + '  <button id="changeUserBtn">change user</button>' + '  <button id="removeUserBtn">remove user</button>' + '  <button id="addUserToAuction">add/remove user to/from auction</button>' + '  </div> ');
    }
  });
  $(document).on('click', "#changeUserBtn", function () {
    $.post("/changeUserAction", {
      userName: $(this).closest('li').find('#newUserName').val(),
      amountOfMoney: $(this).closest('li').find('#newAmountOfMoney').val(),
      ID: $(this).closest('li').find('#ID').text()
    }, changeUser($(this).closest('li').find('#userName'), $(this).closest('li').find('#newUserName').val(), $(this).closest('li').find('#amountOfMoney'), $(this).closest('li').find('#newAmountOfMoney').val()), "json");
  });
  ...
  $(document).on('click', '#addUserFormBtn', function () {
    ...
    var response = $.post("/addUserAction", {
      userName: $('#addUserName').val(),
      amountOfMoney: $('#addAmountOfMoney').val()
    }, () => {}, "json");
    $('ol').append('<li>ID:' + response.responseText + '<br>Name:' + $('#addUserName').val() + '<br>amount of money:' + $('#addAmountOfMoney').val() + '<br>Take part in auction? false</li>');
  });
  $(document).on('click', '#removeUserBtn', function () {
    $.post("/removeUserAction", {
  ...
  function userActionAuction(isInAction) {
    if (isInAction.text() == 'true') isInAction.text('false');else isInAction.text('true');
  }
});